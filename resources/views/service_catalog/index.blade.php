@extends('layouts.app')
@section('title', __('Service Catalog'))

@section('content')

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h4 fw-bold mb-0"><i class="bi bi-grid-3x3-gap text-primary me-2"></i>{{ __('Service Catalog') }}</h1>
        <p class="text-muted small mb-0">{{ __('Define services and map them to journey stages') }}</p>
    </div>
...
<div class="row g-4 align-items-start">
                ...
                <table class="table table-hover mb-0 align-middle">
                    <thead>
                        <tr class="bg-light border-bottom">
                            ...
                            <th class="text-muted small fw-600" style="width:22%">{{ __('NAME') }}</th>
                            <th class="text-muted small fw-600" style="width:28%">{{ __('DESCRIPTION') }}</th>
                            <th class="text-muted small fw-600" style="width:10%">{{ __('STATUS') }}</th>
                            <th class="text-muted small fw-600" style="width:10%">{{ __('USES') }}</th>
                            <th class="text-muted small fw-600 pe-4">{{ __('STAGES MAPPED') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="border-bottom" id="svc-row-{{ $svc->id }}">
                            ...
                            <td>
                                <span class="badge {{ $svc->is_active ? 'bg-success' : 'bg-secondary' }}">
                                    {{ $svc->is_active ? __('Active') : __('Inactive') }}
                                </span>
                            </td>
                            <td>
                                ...
                                <div class="d-flex align-items-center gap-2 flex-wrap">
                                    ...
                                    @if(count($mapped))
                                        <div class="d-flex flex-wrap gap-1">
                                            @foreach(array_slice($mapped, 0, 3) as $stageKey)
                                                @php
                                                    $stageLabel = collect(\App\Services\WorkflowService::STAGES)
                                                        ->firstWhere('key', $stageKey)['label'] ?? $stageKey;
                                                @endphp
                                    ...
                                    <div class="d-flex gap-1 ms-auto">
                                        ...
                                        <form action="{{ route('admin.service-catalog.destroy', $svc) }}" method="POST"
                                              onsubmit="return confirm('{{ __('Delete') }} \'{{ $svc->name }}\'?')">
                                            @csrf @method('DELETE')
                                        ...
                                        @else
                                        <span class="text-muted" title="{{ __('Has active uses') }}"><i class="bi bi-lock"></i></span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="text-center text-muted py-5">
                                ...
                                {{ __('No services defined yet. Add your first one →') }}
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="col-lg-5">
        ...
        <div class="bg-white rounded-3 shadow-sm p-4 mb-3" id="create-form">
            <h6 class="fw-bold mb-3"><i class="bi bi-plus-circle text-success me-1"></i>{{ __('Add Service') }}</h6>
            <form action="{{ route('admin.service-catalog.store') }}" method="POST">
                ...
                <div class="mb-2">
                    ...
                    <input type="text" name="name"
                           ...
                           value="{{ old('name') }}" placeholder="{{ __('e.g. Hotel Booking') }}">
                ...
                <div class="row g-2 mb-2">
                    <div class="col-7">
                        <label class="form-label">{{ __('Icon') }} <span class="text-muted fw-normal">(Bootstrap Icons)</span></label>
                ...
                <div class="mb-3">
                    <label class="form-label">{{ __('Map to Stages') }}</label>
                ...
                <button type="submit" class="btn btn-success btn-sm w-100">
                    <i class="bi bi-plus-lg me-1"></i>{{ __('Create Service') }}
                </button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-3 shadow-sm p-4 border border-primary d-none" id="edit-form">
            <h6 class="fw-bold mb-3"><i class="bi bi-pencil text-primary me-1"></i>{{ __('Edit Service') }}</h6>
            <form id="edit-form-inner" method="POST">
                ...
                <div class="d-flex gap-2">
                    ...
                    <button type="button" class="btn btn-outline-secondary btn-sm" onclick="closeEdit()">{{ __('Cancel') }}</button>
                </div>
            </form>
        </div>
