@extends('layouts.app')
@section('title', __('Milestone Types'))

@push('styles')
<style>
.icon-grid { display:grid;grid-template-columns:repeat(auto-fill,minmax(2.4rem,1fr));gap:.35rem;max-height:260px;overflow-y:auto;padding:.5rem;border:1px solid var(--bs-border-color);border-radius:.5rem;background:#fff }
.icon-grid button { display:flex;align-items:center;justify-content:center;width:2.4rem;height:2.4rem;border:1px solid transparent;border-radius:.4rem;background:none;font-size:1.1rem;cursor:pointer;color:#555;transition:background .12s,border-color .12s,color .12s }
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h4 fw-bold mb-0"><i class="bi bi-diagram-3 text-primary me-2"></i>{{ __('Milestone Types') }}</h1>
        <p class="text-muted small mb-0">{{ __('Control the milestones available in the Journey Timeline') }}</p>
    </div>
                ...
                <table class="table table-hover mb-0 align-middle">
                    <thead>
                        <tr class="bg-light border-bottom">
                            ...
                            <th class="text-muted small">{{ __('Icon') }}</th>
                            <th class="text-muted small">{{ __('Label') }}</th>
                            <th class="text-muted small">{{ __('Key') }}</th>
                            <th class="text-muted small">{{ __('Color') }}</th>
                            <th class="text-muted small">{{ __('STATUS') }}</th>
                            <th class="text-muted small pe-4">{{ __('ACTIONS') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="border-bottom {{ $m->is_active ? '' : 'opacity-50' }}">
                            ...
                            <td>
                                @if($m->is_active)
                                    <span class="badge bg-success-subtle text-success border border-success-subtle" style="font-size:.68rem">{{ __('Active') }}</span>
                                @else
                                    <span class="badge bg-secondary-subtle text-secondary border border-secondary-subtle" style="font-size:.68rem">{{ __('Disabled') }}</span>
                                @endif
                            </td>
                            <td class="pe-4">
                                <div class="d-flex gap-1">
                                    ...
                                    <form action="{{ route('admin.milestone-types.destroy', $m) }}" method="POST"
                                          onsubmit="return confirm('{{ __('Delete') }} \'{{ $m->label }}\'?')">
                                        @csrf @method('DELETE')
    ...
    <div class="col-lg-4">
        <div class="page-card" id="formCard">
            <h6 class="fw-bold mb-3" id="formTitle">
                <i class="bi bi-plus-circle text-primary me-1"></i>{{ __('Add Milestone') }}
            </h6>
            ...
            <form id="milestoneForm" action="{{ route('admin.milestone-types.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div class="form-text">{{ __('Click an icon or type its name manually.') }}</div>
                </div>

                <div class="mb-3">
                    <label class="form-label">{{ __('Label') }} <span class="text-danger">*</span></label>
                    <input type="text" name="label" id="labelInput" class="form-control form-control-sm" required>
                </div>
                ...
                <div class="mb-3" id="keyField">
                    ...
                    <div class="form-text">{{ __('Lowercase letters, numbers, underscores only. Cannot be changed later.') }}</div>
                </div>

                <div class="mb-3">
                    <label class="form-label">{{ __('Color') }} <span class="text-danger">*</span></label>
                ...
                <div class="mb-3 d-none" id="activeField">
                    <div class="form-check form-switch">
                        ...
                        <label class="form-check-label small" for="activeInput">{{ __('Active (visible in dropdowns)') }}</label>
                    </div>
                </div>
...
function renderIconGrid(filter = '') {
    ...
    const list   = filter ? ICONS.filter(i => i.includes(filter.toLowerCase())) : ICONS;
    grid.innerHTML = list.map(icon =>
        `<button type="button" title="${icon}" class="${icon === active ? 'active' : ''}"
                 onclick="selectIcon('${icon}')"><i class="bi ${icon}"></i></button>`
    ).join('');
}
...
document.getElementById('iconSearch').addEventListener('input', function() { renderIconGrid(this.value); });

document.getElementById('labelInput').addEventListener('input', function() {
    if (document.getElementById('formMethod').value === 'POST') {
        document.getElementById('keyInput').value = this.value.toLowerCase().replace(/\s+/g,'_').replace(/[^a-z0-9_]/g,'');
    }
});
...
function openEdit(id, label, icon, color, sortOrder, isActive) {
    document.getElementById('formTitle').innerHTML = '<i class="bi bi-pencil text-warning me-1"></i>{{ __("Edit Milestone") }}';
    ...
    document.getElementById('submitBtn').innerHTML = '<i class="bi bi-check-circle me-1"></i>{{ __("Save") }}';
...
function resetForm() {
    document.getElementById('formTitle').innerHTML = '<i class="bi bi-plus-circle text-primary me-1"></i>{{ __("Add Milestone") }}';
    ...
    document.getElementById('submitBtn').innerHTML = '<i class="bi bi-plus-lg me-1"></i>{{ __("Add") }}';
